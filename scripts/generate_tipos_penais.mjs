...
const pendingRows = pendingRecords
  .map((record) => {
    const treatment = record.modulo === "Código Penal"
      ? "Conferir lei criadora, início de vigência e redação atual em fonte oficial; depois enriquecer e liberar para cálculo."
      : record.modulo === "Trânsito"
        ? "Conferir a lei modificadora e a vigência da redação atual do CTB; depois enriquecer e liberar para cálculo."
        : record.modulo === "Crimes ambientais"
          ? "Conferir a redação alterada e a vigência temporal na lei modificadora; distinguir crime de infração administrativa."
          : "Reconsultar a fonte primária, conferir vigência e completar o enriquecimento jurídico.";
    return `| ${record.id} | ${record.modulo} | ${record.dispositivo} | ${record.nomeJuridico} | ${treatment} |`;
  })
  .join("\n");
